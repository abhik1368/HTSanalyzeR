...
An enrichment map is a network facillitating the visualization and interpretation of GSEA results. In an enrichment map, the nodes represent gene sets and the edges denote the Jaccard similarity coefficient between two gene sets. Nodes are colored by the sign of the enrichment scores (red:+, blue: -) and scaled according to the adjusted GSEA p-values (the darker, the more significant). The sizes of nodes are in proportion to the sizes of gene sets, while the width of edges are proportionate to Jaccard coefficients.

The method \Rfunction{viewEnrichMap} of class \Rclass{GSCA} is developed to view an enrichment map for GSEA results of one or multiple gene set collections. As an example, here we use the sample data in the package to plot the enrichment map for the GSEA results of a KEGG gene set collection.

<<viewEnrichMap1, fig=TRUE, width=6, height=6, results=hide>>=
data("KcViab_GSCA")
viewEnrichMap(KcViab_GSCA, gscs=c("PW_KEGG"), allSig=TRUE, 
...
<<viewEnrichMap2, fig=TRUE, width=6, height=6, results=hide>>=
KcViab_GSCA<-appendGSTerms(KcViab_GSCA, goGSCs=c("GO_BP",
"GO_MF","GO_CC"), keggGSCs=c("PW_KEGG"))
viewEnrichMap(KcViab_GSCA, gscs=c("PW_KEGG"), allSig=TRUE, 
ntop=NULL, gsNameType="term", displayEdgeLabel=FALSE, 
layout="layout.fruchterman.reingold")
@
