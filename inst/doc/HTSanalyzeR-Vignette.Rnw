...
Having preprocessed the input data and created the interactome, the network analysis can then be performed by calling the function \Rfunction{analyze}. In this function, the argument \Robject{fdr} is a parameter for the statistical analysis performed by the BioNet package (see \cite{Beisser2010} for a guide on how to specify those). The function will plot a figure showing the fitting of the BioNet model to your distribution of p-values, which is a good plot to check the choice of statistics used in this function.  
...
\bibitem[Merico {\em et al.}(2010)]{Merico2010}
