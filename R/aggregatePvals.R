...
aggregatePvals<-function(pvalMatrix,method="fishers",pAdjustMethod="BH",order=TRUE){
	...
	if(!(method %in% c("stouffers","fishers") && length(method)==1 )) 
		stop('The argument method should be one of "stouffers" or "fishers"')
	#Check that the argument method has the right format
	if(!(pAdjustMethod %in% c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY","fdr", "none") && length(pAdjustMethod)==1)) 
		stop('The argument method should be one of "stouffers" or "fishers"')
