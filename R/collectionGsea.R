collectionGsea <-
function(collectionOfGeneSets,geneList,exponent=1,nPermutations=1000,minGeneSetSize=15,verbose=TRUE) {
	...
	#and that have more than 'minGeneSetSize' elements that can be found in the geneList	
	...
	if(n.tagGeneSets>0) {
		...
		if(is(getOption("htsa.cluster"),"cluster") && "package:snow" %in% search()) {
			...
			for(i in 1:n.tagGeneSets) {
				scores<-gseaScoresBatch(geneList,geneNames.perm=perm.gL,geneSet=collectionOfGeneSets[[which(tagGeneSets)[i]]],
						exponent=exponent,nPermutations=nPermutations)
