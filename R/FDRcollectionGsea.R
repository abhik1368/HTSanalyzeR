...
#Check that permscores and dataScores have the right format
	...
	if(is.matrix(dataScores)) stop("The argument 'dataScores' should be a vector")
	if(dim(permScores)[1] != length(dataScores)) stop("The number of rows of the permScores matrix should be the same as the length of the dataScores vector")
	if(is.null(names(dataScores))) stop("The dataScores vector should be named (by gene set identifier)")
	print(date())
...
#If the score is 0, just do not normalize anything	
		if(is.na(dataScores[i])){
			NegAvg<-1; PosAvg<-1
				...
				if(dataScores[i]==0) {NegAvg<-1; PosAvg<-1}
				}
