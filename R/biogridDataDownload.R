...
function(link="http://thebiogrid.org/downloads/archives/Release%20Archive/BIOGRID-3.0.64/BIOGRID-ORGANISM-3.0.64.tab2.zip",
	species=c("Drosophila_melanogaster","Homo_sapiens","Rattus_norvegicus","Mus_musculus","Caenorhabditis_elegans")) {
...
#create a directory where the data will be downloaded
	dataDirectory<-file.path(getwd(),"BiogridData")
...
#download the data from the BioGRID and put the compressed file into the above directory, then unzip the file into the directory created above	
	download.file(url=link,destfile=file.path(dataDirectory,"Biogrid-all-org"))
...
#the next few lines find and read the file that contains the data for the species that we want to use
	...
	biogridSpecies<-read.table(file=file.path(dataDirectory,grep(species,listfiles,value=TRUE)),header=TRUE,sep="\t",
