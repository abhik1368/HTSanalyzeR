...
HTSanalyzeR4cellHTS2 <- function(
	...
	data4enrich<-data4enrich[which(!is.na(names(data4enrich)) & !is.na(data4enrich) )]
	paraCheck(cutoffHitsEnrichment)
	if(cutoffHitsEnrichment >= max(data4enrich) && cutoffHitsEnrichment >= abs(min(data4enrich))) {
		stop(paste("The cutoffHitsEnrichment argument should be comprised between 0 and ",max(max(data4enrich),abs(min(data4enrich)))))
	}
	...
	##preprocessing of input gene list and hit list * remove NA; duplicate operations; annotation conversions; order phenotypes
	gsca<-preprocess(gsca,species=species,initialIDs=initialIDs, keepMultipleMappings=TRUE, duplicateRemoverMethod="max", orderAbsValue=FALSE, verbose=verbose)
	##do analysis
	gsca<-analyze(gsca,para=list(pValueCutoff = pValueCutoff,pAdjustMethod = pAdjustMethod, 
					nPermutations = nPermutations, minGeneSetSize = minGeneSetSize,exponent = exponent), verbose=verbose)
	################################
	#2. 	Network analysis
	################################	
	test.stats<-cellHTS2OutputStatTests(cellHTSobject=normCellHTSobject, annotationColumn=annotationColumn, alternative=nwStatsAlternative,controls=nwStatsControls, tests=nwStatsTests)
	...
	#preprocessing
	nwa<-preprocess(nwa,species=species,initialIDs=initialIDs, keepMultipleMappings=TRUE, duplicateRemoverMethod=duplicateRemoverMethod, verbose=verbose)
	#create an interactome
	nwa<-interactome(nwa,interactionMatrix=interactionMatrix, species=species,reportDir=reportDir,genetic=nwAnalysisGenetic, verbose=verbose)
	#do analysis
	nwa<-analyze(nwa, fdr=nwAnalysisFdr, species=species, verbose=verbose)	
	################################
	#3. 	write report
	################################	
	reportAll(gsca=gsca, nwa=nwa, experimentName=deparse(substitute(normCellHTSobject)), species=species, ntop=ntop, allSig=allSig, 
			keggGSCs=keggGSCs, goGSCs=goGSCs, reportDir=reportDir)	
	#save GSCA and NWA objects to RData
	save(gsca, nwa, file=file.path(reportDir,paste(deparse(substitute(normCellHTSobject)),".RData",sep="")))
}