...
writeReportHTSA <-
function(experimentName,enrichmentAnalysis,cutoffHits,hits,listOfGeneSetCollections,geneList,geneListName,p.adj.method="BH",
		nPermutations=1000,min.gene.set.size=15,exponent=1,nwAnalysisOutput,controls="neg",
		alternative="two.sided",tests=c("T-test"),columns=c("t.test.pvalues.two.samples","t.test.pvalues.one.sample"),species=c("Dm"),
		fdr=0.001,genetic=FALSE,networkObject=NA,nGseaPlots=10,whichSetIsKEGGIds=NA,whichSetIsGOIds=NA,reportdir="HTSanalyzerReport"){
...
#!mod: file and directory creation
	
	docdir<-file.path(reportdir,"doc")
	imgdir<-file.path(reportdir,"image")
	htmdir<-file.path(reportdir,"html")
...
#1.for the non-GO and non-KEGG gene set collections	
		if(length(intersect(s,keggGS)) == 0 && length(intersect(s,GOGS)) == 0){
			for(p in 1:nGseaPlots){
				...
				test<-gseaScores(geneList=geneList,geneSet=(listOfGeneSetCollections[[s]][[rownames(enrichmentAnalysis$GSEA.results[[names(listOfGeneSetCollections)[s]]])[p]]]),exponent=1,mode="graph")
				gseaPlots(Running.Score=test$Running.Score,Enrichment.Score=test$Enrichment.Score,Positions=test$Positions,geneList=geneList,output="png",filepath=imgdir,filename=t.name)
			}
		}	
#2.for the GO gene set collections		
		if(length(intersect(s,GOGS)) != 0){
			for(p in 1:nGseaPlots){
				...
				test<-gseaScores(geneList=geneList,geneSet=(listOfGeneSetCollections[[s]][[GOgsname]]),exponent=1,mode="graph");
				gseaPlots(Running.Score=test$Running.Score,Enrichment.Score=test$Enrichment.Score,Positions=test$Positions,geneList=geneList,output="png",filepath=imgdir,filename=GOgsname);
				}
		}	
#3.for the KEGG gene set collections		
		if(length(intersect(s,keggGS)) != 0){
			for(p in 1:nGseaPlots){
				kegggsname<-rownames(enrichmentAnalysis$GSEA.results[[names(listOfGeneSetCollections)[s]]])[p]
				...
				test<-gseaScores(geneList=geneList,geneSet=(listOfGeneSetCollections[[s]][[kegggsname]]),exponent=1,mode="graph");
				gseaPlots(Running.Score=test$Running.Score,Enrichment.Score=test$Enrichment.Score,Positions=test$Positions,geneList=geneList,output="png",filepath=imgdir,filename=kegggsname);
			}
		}	
}

#Also, produce files containing the overlap between hits and gene sets, to be linked to the hypergeometric results pages
	if(species == "Dm"){mapID<-as.list(org.Dm.egSYMBOL)}
	if(species == "Hs"){mapID<-as.list(org.Hs.egSYMBOL)}
	if(species == "Rn"){mapID<-as.list(org.Rn.egSYMBOL)}
	if(species == "Mm"){mapID<-as.list(org.Mm.egSYMBOL)}
	if(species == "Ce"){mapID<-as.list(org.Ce.egSYMBOL)}
...
#1.for the non-GO and non-KEGG gene set collections	
		if(length(intersect(s,keggGS)) == 0 && length(intersect(s,GOGS)) == 0){
			for(p in 1:nGseaPlots){
				...
				overlap<-intersect((listOfGeneSetCollections[[s]][[rownames(enrichmentAnalysis$HyperGeo.results[[names(listOfGeneSetCollections)[s]]])[p]]]),hits)
				overlapSymbols<-rep(0,length(overlap))
				if(is.list(mapID) == TRUE && length(overlapSymbols) != 0) {
					for(g in 1:length(overlapSymbols)){
						if(length(mapID[[overlap[g]]]) == 1) {overlapSymbols[g]<-mapID[[overlap[g]]]}
						}
					}
				filename<-file.path(docdir,paste(t.name,".txt",sep=""))
...
#2.for the GO gene set collections		
		if(length(intersect(s,GOGS)) != 0){
			for(p in 1:nGseaPlots){
				...
				overlap<-intersect((listOfGeneSetCollections[[s]][[GOgsname]]),hits)
				overlapSymbols<-rep(0,length(overlap))
				if(is.list(mapID) == TRUE && length(overlapSymbols) != 0) {
					for(g in 1:length(overlapSymbols)){
						if(length(mapID[[overlap[g]]]) == 1) {overlapSymbols[g]<-mapID[[overlap[g]]]}
				...
				filename<-file.path(docdir,paste(GOgsname,".txt",sep=""))
				overlap<-cbind(overlap,overlapSymbols)
				colnames(overlap)<-c("EntrezGene","Symbols")
				write.table(overlap,file=filename,row.names=FALSE,quote=FALSE)
			}
...
#3.for the KEGG gene set collections		
		if(length(intersect(s,keggGS)) != 0){
			for(p in 1:nGseaPlots){
				...
				#kegggsname<-sub(pattern="( :: \\D*$)",replacement="",x=kegggsname,perl=TRUE)
				#kegggsname<-sub(pattern="( :: \\w*$)",replacement="",x=kegggsname,perl=TRUE)
				kegggsname<-rownames(enrichmentAnalysis$HyperGeo.results[[names(listOfGeneSetCollections)[s]]])[p]
				overlap<-intersect((listOfGeneSetCollections[[s]][[kegggsname]]),hits)
				overlapSymbols<-rep(0,length(overlap))
				if(is.list(mapID) == TRUE && length(overlapSymbols) != 0) {
					for(g in 1:length(overlapSymbols)){
						if(length(mapID[[overlap[g]]]) == 1) {overlapSymbols[g]<-mapID[[overlap[g]]]}
				...
				filename<-file.path(docdir,paste(kegggsname,".txt",sep=""))
				overlap<-cbind(overlap,overlapSymbols)
				colnames(overlap)<-c("EntrezGene","Symbols")
				write.table(overlap,file=filename,row.names=FALSE,quote=FALSE)
			}
...
#Produce the index html page
	...
	cat('\n <html> \n <link rel="stylesheet" href="image/htsanalyzer.css" type="text/css"> ',append = TRUE, file = htmlfile)
	...
	cat('<td class="main"> <table> <tr> <div class="HTSlogos"> <img src="image/Rlogo.png" width="50" height="40"/>',append = TRUE,file=htmlfile)
	cat('&nbsp <img src="image/blue_cruklogo.gif" width="120" height="50"/>',append = TRUE,file=htmlfile)
	cat('&nbsp <img src="image/goatcomputer.png" width="85" height="60"/> </div></tr><tr>',append = TRUE,file=htmlfile)
#Produce the tabs	
	...
	for(i in 1:l.HyperGeo.results){
		cat(paste('\n <td class="tabs"> <h3><a href="html/hyperg',i,'.html" title="',sep=""), append = TRUE, file = htmlfile)
	...
	for(i in 1:l.GSEA.results){
		cat(paste('\n <td class="tabs"> <h3><a href="html/gsea',i,'.html" title="',sep=""), append = TRUE, file = htmlfile)
	...
	for(i in 1:l.Sig.adj.pvals.in.both){
			cat(paste('\n <td class="tabs"> <h3><a href="html/enrichment',i,'.html" title="',sep=""), append = TRUE, file = htmlfile)
			cat(paste(names(enrichmentAnalysis$Sig.adj.pvals.in.both)[i],' Enrichment.summary">','here</a></h3> </td>',sep=""), append = TRUE, file = htmlfile)
			}
	cat("\n </tr> <tr>", append = TRUE, file = htmlfile)
	cat(paste('\n  <td class="tabs"> <h3><a href="html/network.html" title="network">Network Analysis</a></h3> </td> </tr> </table>',sep=""), append = TRUE, file = htmlfile)
...
#Produce the main data frame: headers and first row	
		...
		cat(paste('<a href="../doc/',t.name,'.txt" target="_blank" title="Observed.hits">',signif(enrichmentAnalysis$HyperGeo.results[[n]][1,5], digits=4),'</a> </td> \n <td>', sep=""), append = TRUE, file = htmlfile)
		if(enrichmentAnalysis$HyperGeo.results[[n]][1,8] > 0.05){
...
#Produce the main dataframe: rows with plots
	...
	if((i%%2) == 0){
		...
		cat(paste('<a href="../doc/',t.name,'.txt" target="_blank" title="Observed.hits">',signif(enrichmentAnalysis$HyperGeo.results[[n]][i,5], digits=4),'</a> </td> \n <td>', sep=""), append = TRUE, file = htmlfile)
		if(enrichmentAnalysis$HyperGeo.results[[n]][i,8] > 0.05){
...
#Produce the main data frame: rows without plots			
	...
	if((i%%2) == 0){
		...
		signif(enrichmentAnalysis$HyperGeo.results[[n]][i,6], digits=4),sep=""),append = TRUE, file = htmlfile)
		if(enrichmentAnalysis$HyperGeo.results[[n]][i,8] > 0.05) {
			cat(paste('</td> \n <td>', signif(enrichmentAnalysis$HyperGeo.results[[n]][i,8], digits=4),"</td> </tr>", sep=""),append = TRUE, file = htmlfile)
			}else{
				cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$HyperGeo.results[[n]][i,8], digits=4),"</td> </tr>", sep=""),append = TRUE, file = htmlfile)
				}
		}else{
		cat(paste('<tr class="odd"> <td>',rownames(enrichmentAnalysis$HyperGeo.results[[n]])[i],"</td> \n <td>",
		signif(enrichmentAnalysis$HyperGeo.results[[n]][i,1], digits=4),"</td> \n <td>",
...
#Produce the main data frame
		kegggsname<-rownames(enrichmentAnalysis$HyperGeo.results[[n]])[1]
		...
		signif(enrichmentAnalysis$HyperGeo.results[[n]][1,3], digits=4),"</td> \n <td>",
		signif(enrichmentAnalysis$HyperGeo.results[[n]][1,4], digits=4),"</td> \n <td>", sep=""), append = TRUE, file = htmlfile)
		cat(paste('<a href="../doc/',kegggsname,'.txt" target="_blank" title="Observed.hits">',signif(enrichmentAnalysis$HyperGeo.results[[n]][1,5], digits=4),'</a> </td> \n <td>', sep=""), append = TRUE, file = htmlfile)
		if(enrichmentAnalysis$HyperGeo.results[[n]][1,8] > 0.05){
			cat(paste(signif(enrichmentAnalysis$HyperGeo.results[[n]][1,6], digits=4),"</td> \n <td>",
			signif(enrichmentAnalysis$HyperGeo.results[[n]][1,8], digits=4),"</td> \n </tr>", sep=""), append = TRUE, file = htmlfile)
			}else{			
...
#Rows with a plot		
	for(i in 2:nGseaPlots){
		kegggsname<-rownames(enrichmentAnalysis$HyperGeo.results[[n]])[i]
	...
	if((i%%2) == 0){
		...
		signif(enrichmentAnalysis$HyperGeo.results[[n]][i,3], digits=4),"</td> \n <td>",
		signif(enrichmentAnalysis$HyperGeo.results[[n]][i,4], digits=4),"</td> \n <td>", sep=""), append = TRUE, file = htmlfile)
		cat(paste('<a href="../doc/',kegggsname,'.txt" target="_blank" title="Observed.hits">',signif(enrichmentAnalysis$HyperGeo.results[[n]][i,5], digits=4),'</a> </td> \n <td>', sep=""), append = TRUE, file = htmlfile)
		if(enrichmentAnalysis$HyperGeo.results[[n]][i,8] > 0.05){
			cat(paste(signif(enrichmentAnalysis$HyperGeo.results[[n]][i,6], digits=4),"</td> \n <td>",
			signif(enrichmentAnalysis$HyperGeo.results[[n]][i,8], digits=4),"</td> \n </tr>", sep=""), append = TRUE, file = htmlfile)
			}else{
...
#Rows without plot		
	...
	if((i%%2) == 0){
		...
		signif(enrichmentAnalysis$HyperGeo.results[[n]][i,6], digits=4),sep=""), append = TRUE, file = htmlfile)
		if(enrichmentAnalysis$HyperGeo.results[[n]][i,8] > 0.05){
			cat(paste('</td> \n <td>',signif(enrichmentAnalysis$HyperGeo.results[[n]][i,8], digits=4),"</td> \n </tr>", sep=""), append = TRUE, file = htmlfile)
			}else{
				cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$HyperGeo.results[[n]][i,8], digits=4),"</td> \n </tr>", sep=""), append = TRUE, file = htmlfile)
				}
		}else{
		cat(paste('<tr class="odd"> <td><a href="http://www.genome.jp/dbget-bin/www_bget?pathway:',kegggsname,'" target="_blank" title="',rownames(enrichmentAnalysis$HyperGeo.results[[n]])[i],'">',rownames(enrichmentAnalysis$HyperGeo.results[[n]])[i],'</a> </td> \n <td>',
		signif(enrichmentAnalysis$HyperGeo.results[[n]][i,1], digits=4),"</td> \n <td>",
...
#Produce the main dataframe: headers and first row	
		...
		cat(paste('\n <hr/> \n <br>',names(enrichmentAnalysis$GSEA.results)[n],' GSEA ' ,'\n <br> \n <table class="results"> <tr class="head"> <th>Gene Set name</th> <th>',
			...
			signif(enrichmentAnalysis$GSEA.results[[n]][1,2], digits=4), sep=""), append = TRUE, file = htmlfile)
if(enrichmentAnalysis$GSEA.results[[n]][1,3] > 0.05){
				cat(paste("</td> \n <td>",signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
				signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
					'<a href="../image/gsea_plots',t.name,'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""), append = TRUE, file = htmlfile)
				}else{
					cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
						'<a href="../image/gsea_plots',t.name,'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""), append = TRUE, file = htmlfile)					
					}
...
#Produce the main dataframe: rows with plots	
		for(i in 2:nGseaPlots){
			...
			if((i%%2) == 0){
				...
				signif(enrichmentAnalysis$GSEA.results[[n]][i,2], digits=4), sep=""),append = TRUE, file = htmlfile)
if(enrichmentAnalysis$GSEA.results[[n]][i,3] > 0.05){
					cat(paste("</td> \n <td>",signif(enrichmentAnalysis$GSEA.results[[n]][i,3], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][i,4], digits=4),"</td> \n <td>",
						'<a href="../image/gsea_plots',t.name,'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""),append = TRUE, file = htmlfile)
						}else{
						cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$GSEA.results[[n]][i,3], digits=4),"</td> \n <td>",
						signif(enrichmentAnalysis$GSEA.results[[n]][i,4], digits=4),"</td> \n <td>",
							'<a href="../image/gsea_plots',t.name,'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""),append = TRUE, file = htmlfile)
							}
				}else{
...
#Produce the tabs
	...
	cat(paste('\n  <td class="tabs"> <h3><a href="./network.html" title="network">Network Analysis</a></h3> </td> </tr> </table>',sep=""), append = TRUE, file = htmlfile)
#Produce the main dataframe: headers and first row	
		kegggsname<-rownames(enrichmentAnalysis$GSEA.results[[n]])[1]
		...
		cat(paste('\n <hr/> \n <br>',names(enrichmentAnalysis$GSEA.results)[n],' GSEA ' ,'\n <br> \n <table class="results"> <tr class="head"> <th>Gene Set name</th> <th>',
			...
			if(enrichmentAnalysis$GSEA.results[[n]][1,3] > 0.05){
				cat(paste("</td> \n <td>",signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
				signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
					'<a href="../image/gsea_plots',kegggsname,'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""), append = TRUE, file = htmlfile)
				}else{
					cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
						'<a href="../image/gsea_plots',kegggsname,'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""), append = TRUE, file = htmlfile)
...
#Produce the main dataframe: rows with plots	
		for(i in 2:nGseaPlots){
			kegggsname<-rownames(enrichmentAnalysis$GSEA.results[[n]])[i]
...
##Produce the main dataframe: headers and first row	
		cat(paste('\n <hr/> \n <br>',names(enrichmentAnalysis$GSEA.results)[n],' GSEA ' ,'\n <br> \n <table class="results"> <tr class="head"> <th>Gene Set name</th> <th>',
			...
			if(enrichmentAnalysis$GSEA.results[[n]][1,3] > 0.05){
				cat(paste("</td> \n <td>",signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
				signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
					'<a href="../image/gsea_plots',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[1],perl=TRUE),'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""), append = TRUE, file = htmlfile)
				}else{
					cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$GSEA.results[[n]][1,3], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][1,4], digits=4),"</td> \n <td>",
						'<a href="../image/gsea_plots',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[1],perl=TRUE),'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""), append = TRUE, file = htmlfile)
		...
		for(i in 2:nGseaPlots){
			if((i%%2) == 0){
				...
				if(enrichmentAnalysis$GSEA.results[[n]][i,3] > 0.05){
					cat(paste("</td> \n <td>",signif(enrichmentAnalysis$GSEA.results[[n]][i,3], digits=4),"</td> \n <td>",
					signif(enrichmentAnalysis$GSEA.results[[n]][i,4], digits=4),"</td> \n <td>",
						'<a href="../image/gsea_plots',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[i],perl=TRUE),'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""),append = TRUE, file = htmlfile)
					}else{
						cat(paste('</td> \n <td class="signif">',signif(enrichmentAnalysis$GSEA.results[[n]][i,3], digits=4),"</td> \n <td>",
						signif(enrichmentAnalysis$GSEA.results[[n]][i,4], digits=4),"</td> \n <td>",
							'<a href="../image/gsea_plots',sub(pattern="(\\D*$)",replacement="",x=rownames(enrichmentAnalysis$GSEA.results[[n]])[i],perl=TRUE),'.png" target="_blank" title="gseaplots">plot</a> ',"</td> </tr>", sep=""),append = TRUE, file = htmlfile)
...
#Produce a link to the graph file containing the module	
	cat(paste('\n <hr/> \n  <br>Click <a href="../image/',nwAnalysisGraphFile,'" target="_blank" title="Enriched Subnetwork">here</a> to get the enriched subnetwork ',sep=""), append = TRUE, file = htmlfile)
#Check that the nwAnalysisOutput has the right format
	...
	cat(paste('</table> </td> <td> <img src="../image/',nwAnalysisGraphFile,'" align="top" width="800" height="800"> </td> </tr> </table>',sep=""),append = TRUE, file = htmlfile)	
	cat('\n </tr> \n </table> \n </td> \n </tr> \n </table> \n </body> \n </html>',append = TRUE, file = htmlfile)
}
