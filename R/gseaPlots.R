...
gseaPlots <-
	function(Running.Score,Enrichment.Score,Positions,geneList,output="png",filepath,filename) {
	#check that the geneList is a single numerical vector
	if(!is.numeric(geneList) && !is.integer(geneList)) 
		stop("The geneList should be a single vector of numerical phenotypes")
	#check that the 'Running.Score' is a vector of length=length of geneList
	if(!is.numeric(Running.Score)) 
		stop("The Running.Score should be a numerical vector")
	if(length(Running.Score) != length(geneList)) 
		warning("The length of the Running.Score vector should be the same as the length of the geneList")
	...
	if(length(Positions) != length(geneList)) 
		warning("The length of the Positions vector should be the same as the length of the geneList")
	if(!(is.numeric(Positions) || is.integer(Positions))) 
		stop("The Positions vector should be a numerical vector")
	if(!all(unique(Positions) %in% c(0,1))) 
		warning("The Positions vector should contains only 0s and 1s, see help(gseaScores)")
	...
	if(!is.character(output) || !(output %in% c("png","pdf"))) 
	...
	par(mai=c(1,1,0.1,1))
	plot(x=c(1:length(Running.Score)),y=Running.Score,type="l",xlab="Position in the ranked list of genes",ylab="Running enrichment score")
	abline(h=0)
