...
pairwiseGseaPlot<-function(gl1,gl2,geneSet,exponent=1,output="png",filepath,filename){
	...
	plot(x=c(1:length(gseaPh1$runningScore)),y=gseaPh1$runningScore,type="l",xlab="Position in the ranked list of genes",ylab="Running enrichment score")
	abline(h=0)
	abline(v=which(gseaPh1$runningScore == gseaPh1$enrichmentScore),lty=3,col="magenta",lwd=3)	
	par(mai=c(1,0.8,0.1,0.1))
	plot(x=c(1:length(gseaPh2$runningScore)),y=gseaPh2$runningScore,type="l",xlab="Position in the ranked list of genes",ylab="Running enrichment score")
	abline(h=0)
	abline(v=which(gseaPh2$runningScore == gseaPh2$enrichmentScore),lty=3,col="magenta",lwd=3)
	dev.off()
}
