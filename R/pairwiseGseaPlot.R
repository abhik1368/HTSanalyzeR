...
pairwiseGseaPlot<-function(gl1,gl2,geneSet,exponent=1,name,output="png"){
...
#open a file	
	if(output == "pdf" ) {
		pdf(paste("pairwiseGsea_plots",name,".pdf",sep=""))
		}
	if(output == "png" ) {
		png(paste("pairwiseGsea_plots",name,".png",sep=""))
		}
...
#this is done using the 'Positions' output of gseaScores, which stores a one for each match position and a zero otherwise	
	plot(x=seq(1,length(gl1)),type="l",y=gl1,ylab="Phenotypes",xlab=NA,
		col="magenta",lwd=2,xaxt="n",main="Phenotype 1")
	abline(v=which(gseaPh1$Positions == 1))
	abline(h=0)
	par(mai=c(0.2,0.8,0.5,0.1))
	plot(x=seq(1,length(gl2)),type="l",y=gl2,ylab="Phenotypes",xlab=NA,
		col="magenta",lwd=2,xaxt="n",main="Phenotype 2")
...
#Plot the running score and add a vertical line at the position of the enrichment score (maximal absolute value of the running score)	
	...
	plot(x=c(1:length(gseaPh1$Running.Score)),y=gseaPh1$Running.Score,type="l",
	...
	par(mai=c(1,0.8,0.1,0.1))
	plot(x=c(1:length(gseaPh2$Running.Score)),y=gseaPh2$Running.Score,type="l",
		xlab="Position in the ranked list of genes",ylab="Running enrichment score")
	abline(h=0)
