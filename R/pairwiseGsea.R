...
pairwiseGsea<-function(gl1,gl2,gsc,exponent=1,nperm=1000,cutoff=15,MHT="BH") {
	...
	if(length(unique(names(gl1), names(gl2))) != length(gl1)) 
		stop("The two phenotypes must be measured on the same genes")
	if(length(intersect(names(gl1), names(gl2))) != length(gl1)) 
		stop("The two phenotypes must be measured on the same genes")
	...
	collGsea.ph1<-collectionGsea(collectionOfGeneSets=gsc,geneList=gl1,exponent=exponent,npermutations=nperm,cutoffGeneSetSize=cutoff)
	collGsea.ph2<-collectionGsea(collectionOfGeneSets=gsc,geneList=gl2,exponent=exponent,npermutations=nperm,cutoffGeneSetSize=cutoff)
