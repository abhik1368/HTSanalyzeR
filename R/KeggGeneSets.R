...
function(species=c("Dm", "Hs", "Rn", "Mm", "Ce")) {
		...
		if(species=="Hs") this.pw.kegg<-Kegg[grep("hsa",names(Kegg))]
		if(species=="Mm") this.pw.kegg<-Kegg[grep("mmu",names(Kegg))]
		if(species=="Rn") this.pw.kegg<-Kegg[grep("rno",names(Kegg))]
		if(species=="Ce") this.pw.kegg<-Kegg[grep("cel",names(Kegg))]
		...
		if(species=="Dm") {
			...
			junk<-sapply(1:length(this.pw.kegg), function(l) {this.pw.kegg[[l]]<<-unlist(sapply(this.pw.kegg[[l]], function(i) {if(length(fbgn2eg[[i]])>0) fbgn2eg[[i]][1]}))})
		}
		return(this.pw.kegg)
}
