...
function(geneList,geneNames.perm,collectionOfGeneSets,exponent,npermutations, mode=c("graph","score")) {
	...
	scores<-parSapply(getOption("htsa.cluster"), 1:length(collectionOfGeneSets), 
			function(i) {
				gseaScoresBatchLocal(geneList,geneNames.perm=geneNames.perm,geneSet=as.integer(collectionOfGeneSets[[i]]), exponent=exponent,npermutations=npermutations, mode=mode)
			}
	)
	return(scores)
}
